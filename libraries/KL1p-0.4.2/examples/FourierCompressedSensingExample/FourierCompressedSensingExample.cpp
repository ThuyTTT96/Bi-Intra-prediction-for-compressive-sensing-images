...
#include "FourierCompressedSensingExample.h"
...
void	kl1p::RunExample()
{
	try
	{
		std::cout<<"Start of KL1p Fourier compressed-sensing example."<<std::endl;
		std::cout<<"Try to determine a sparse vector x "<<std::endl;
		std::cout<<"from an underdetermined set of linear measurements y=S*F*x, "<<std::endl;
		std::cout<<"where S is a random measurement matrix and F a Fourier matrix."<<std::endl;
		...
		klab::TSmartPointer<kl1p::TOperator<std::complex<klab::DoubleReal> > > F = new kl1p::TFourier1DOperator<std::complex<klab::DoubleReal> >(n);	// Create Fourier operator.
		klab::TSmartPointer<kl1p::TOperator<std::complex<klab::DoubleReal> > > S = new kl1p::TRandomDownSamplingOperator<std::complex<klab::DoubleReal> >(m, n);	// Create random measurement operator.
		klab::TSmartPointer<kl1p::TOperator<std::complex<klab::DoubleReal> > > A = S * F;	//Create the sensing operator.	
		A  = new kl1p::TScalingOperator<std::complex<klab::DoubleReal> >(A, 1.0/klab::Sqrt(klab::DoubleReal(m)));	// Pseudo-normalization of the matrix (required for AMP and EMBP solvers).		
		...
		timer.start();
		kl1p::TOMPSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > omp(tolerance);
		omp.solve(y, A, k, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
		...
		timer.start();
		kl1p::TROMPSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > romp(tolerance);
		romp.solve(y, A, k, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
		...
		timer.start();
		kl1p::TCoSaMPSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > cosamp(tolerance);
		cosamp.solve(y, A, k, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
			x[i] = cx[i].real();
		...
		timer.start();
		kl1p::TSubspacePursuitSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > sp(tolerance);
		sp.solve(y, A, k, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
		...
		timer.start();
		kl1p::TSL0Solver<klab::DoubleReal, std::complex<klab::DoubleReal> > sl0(tolerance);
		sl0.solve(y, A, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
		...
		timer.start();
		kl1p::TAMPSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > amp(tolerance);
		amp.solve(y, A, cx);
		timer.stop();
		x.set_size(cx.n_rows);
		for(klab::UInt32 i=0; i<x.n_rows; ++i)
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal, std::complex<klab::DoubleReal> > embp(tolerance);
