...
#include "SeededEMBPCompressedSensingExample.h"
...
void	kl1p::RunExample()
{
	try
	{
		std::cout<<"Start of KL1p seeded-EMBP compressed-sensing example."<<std::endl;
		...
		std::cout<<"where A is a random block sensing matrix."<<std::endl;

		klab::UInt32 n = 256;					// Size of the original signal x0.
		klab::DoubleReal alpha = 0.35;			// Ratio of the cs-measurements.
		...
		klab::UInt32 nb = n / blocks;			// Size of the blocks of the sensing matrix.
		klab::DoubleReal alpha0 = 1.0;			// Ratio of the measurements in the first block of the sensing matrix.
		...
		klab::TSmartPointer<kl1p::TOperator<klab::DoubleReal> > A0 = new kl1p::TNormalRandomMatrixOperator<klab::DoubleReal>(m, n, 0.0, 1.0);
		A0 = new kl1p::TScalingOperator<klab::DoubleReal>(A0, 1.0/klab::Sqrt(klab::DoubleReal(A0->m())));	// Pseudo-normalization of the matrix (required for homogeneous EMBP solver).		

		// Create random seeding matrix A of size(m, n).
		klab::TSmartPointer<kl1p::TBlockOperator<klab::DoubleReal> > A = new kl1p::TSeedingOperator<klab::DoubleReal>(n, blocks, alpha0, alphaB, dvariance, lvariance, uvariance);

		// Use seeding matrix as standard matrix for homogeneous version of EMBP solver.
		klab::TSmartPointer<kl1p::TOperator<klab::DoubleReal> > Ah = klab::static_pointer_cast<kl1p::TOperator<klab::DoubleReal> >(A);
		Ah = new kl1p::TScalingOperator<klab::DoubleReal>(Ah, 1.0/klab::Sqrt(klab::DoubleReal(Ah->m())));	// Pseudo-normalization of the matrix (required for homogeneous EMBP solver).		
		...
		// created from a choosen operator (a Walsh-Hadamard operator in the example below).
		klab::TSmartPointer<TOperator<klab::DoubleReal> > op = new TScalingOperator<klab::DoubleReal>(new TWalshHadamard1DOperator<klab::DoubleReal>(nb), klab::Sqrt(1.0/n));
		klab::TSmartPointer<kl1p::TBlockOperator<klab::DoubleReal> > Ag = new kl1p::TGenericSeedingOperator<klab::DoubleReal>(op, blocks, klab::UInt32(alpha0*nb), klab::UInt32(alphaB*nb), dvariance, lvariance, uvariance);

		// Use generic-seeding matrix as standard matrix for homogeneous version of EMBP solver.
		klab::TSmartPointer<kl1p::TOperator<klab::DoubleReal> > Agh = klab::static_pointer_cast<kl1p::TOperator<klab::DoubleReal> >(Ag);
		Agh = new kl1p::TScalingOperator<klab::DoubleReal>(Agh, 1.0/klab::Sqrt(klab::DoubleReal(Agh->m())));	// Pseudo-normalization of the matrix (required for homogeneous EMBP solver).		
		...
		arma::Col<klab::DoubleReal> yg;
		arma::Col<klab::DoubleReal> yh;
		A0->apply(x0, yh);
		A->apply(x0, y);
		Ag->apply(x0, yg);
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal> embph0(tolerance);
		embph0.enableHomogeneous(true);
		embph0.solve(yh, A0, k, x);
		timer.stop();
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal> embph1(tolerance);
		embph1.enableHomogeneous(true);
		embph1.solve(y, Ah, k, x);
		timer.stop();
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal> embps(tolerance);
		embps.solve(y, A, k, x);
		timer.stop();
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal> embph2(tolerance);
		embph2.enableHomogeneous(true);
		embph2.solve(yg, Agh, k, x);
		timer.stop();
		...
		timer.start();
		kl1p::TEMBPSolver<klab::DoubleReal> embpgs(tolerance);
		embpgs.solve(yg, Ag, k, x);
		timer.stop();
