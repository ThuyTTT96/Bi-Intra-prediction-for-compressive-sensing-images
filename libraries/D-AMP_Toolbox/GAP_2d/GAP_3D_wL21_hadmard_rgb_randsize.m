function theta_3d_out = GAP_3D_wL21_hadmard_rgb_randsize(y,Row,Col,T, block,spbasis, ydim,stopc,acc,weight_base,weighttype)
    % Xin Yuan, duke ECE
    % xin.yuan@duke.edu
...
theta_3d = zeros(2^(ceil(log2(Row*Col))),T);
    ...
    theta_3d_old = theta_3d; % Used to check the stop criterion
     ...
     %theta_3d = reshape(theta,[row, col,T]);
       y0_2d  = y;
        ...
        theta_temp_3d(:,:,nr) = reshape(theta_temp(1:Row*Col,nr),[Row, Col]);
        end
        theta_temp_3d_2 = zeros(row, col, T);
        theta_temp_3d_2(1:Row, 1:Col,:) =theta_temp_3d; 
        
        w_3d = myfasttransform3_givenT(theta_temp_3d_2,T_row,T_col,T_t);  % Note here theta_temp and w are all 3D
       ...
       theta_3d_out1 = reshape(theta_3d, [row, col, T_new]);
      % theta_3d_out2 = zeros(Row, Col, T_new);
       theta_3d_out2= theta_3d_out1(1:Row, 1:Col,:);
       theta_3d_out3 = reshape(theta_3d_out2,[Row*Col, T_new]);
       theta_3d = zeros(2^(ceil(log2(Row*Col))),T);
      
       
        for nr = 1:T
       theta_3d(1:Row*Col,nr) =  theta_3d_out3(1:Row*Col,nr);
...
for nr = 1:T
   theta_3d_out(:,:,nr) = reshape(theta_3d(1:Row*Col,nr), [Row, Col]);
end
