function weight = my_get_weight3d_tree_exp(row,col,T,block,weight_base)

% Xin Yuan, get the weight of coefficient for GAP 3D
% initial date: 04/30/2013
% Revised date: 05/04/2013, add the zero-tree structure of the weight for wavelet tree
...
for nt = 1:block_num_T
    if(nt*T_block<=T)
        ...
        %weight(:,:,(nt-1)*T_block+(1:T_block))  = repmat(weight_2D+repmat(weight_T(nt),[row,col]), [1 1 T_block]);
    else
        weight(:,:,((nt-1)*T_block+1):T) = repmat(weight_2D*weight_T(nt), [1, 1, T-((nt-1)*T_block)]);
        %weight(:,:,((nt-1)*T_block+1):T) = repmat(weight_2D+repmat(weight_T(nt),[row,col]), [1, 1, T-((nt-1)*T_block)]);
    end
end
