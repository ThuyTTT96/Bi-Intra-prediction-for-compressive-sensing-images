function [theta model]  = GAP_2D_wL21_tree_hadmard(y0, row,col, block, domain, m_star,stopc,acc)
% Xin Yuan, 
% initial date: 04/30/2015

% Input: y, the 1D measurement
% Input: we don't need to save Phi and Phi' now, since it is given by
% hadmard
...
for iter_i = 1:(iternum)
    ...
    if(acc)
        y = y + (y0 - getPhitheta(theta,ydim));
    else
        y = y0;
    end
    theta_temp = theta+ getPinvPhitheta(y,theta,ydim); %P*(y-Phi*theta);
    ...
    if(size(w_temp_weight_vec,1)>1)
       w_temp_vec_l21 = sqrt(sum(w_temp_weight_vec.^2)); 
    else
         w_temp_vec_l21 = abs(w_temp_weight_vec);
    end
