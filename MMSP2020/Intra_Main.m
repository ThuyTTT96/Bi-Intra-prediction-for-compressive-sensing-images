...
addpath(genpath('C:/Users/Thuy Tran/Documents/Research/Frame/WILD'));
...
%                             y(2:64) = (y(1)/2)-random;
                            ...
                            %___QUANTIZATION___
                        %log2(2)   1-bit quantization
                        %log2(4)   2-bit quantization
                        %log2(8)   3-bit quantization
                        %log2(16)  4-bit quantization
                        %log2(32)  5-bit quantization
                        %log2(64)  6-bit quantization
                        %log2(128) 7-bit quantization
                        %log2(256) 8-bit quantization
                        if(frame_number == 1)
                           M = 32;
                        else
                           M = 32;
                ...
                for i = 1:size(frame,1)/sub_pixels
                    for j = 1:size(frame,2)/sub_pixels
                       if(strcmp(quartus_interface,'on'))
                           ...
                           y_dequantized{i,j,k}  = (y_quantized{i,j,k}*bits_shift{i,j,k}) + 0.5;
        ...
        imwrite(uint8(video_buffer(:,:,:,frame_number)),[num2str(frame_number),'_',num2str(sampling_rate) '.png']);
        imwrite(uint8(res_video_buffer(:,:,:,frame_number)),['residual',num2str(frame_number),'_',num2str(sampling_rate) '.png']);
